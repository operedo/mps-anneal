...
echo "mpirun -n $NSLOTS ${home}/mps-anneal/bin/mps-anneal $dataTI $dataRE $logTI $logRE $topo $out $sched $rand $rand2 $nx $ny $nz $tnx $tny $tnz $hashsize $t0 $lambda $npert $ntemp $maxatt $irepo > outputs/$advance"

mpirun -n $NSLOTS ${home}/mps-anneal/bin/mps-anneal $dataTI $dataRE $logTI $logRE $topo $out $sched $rand $rand2 $nx $ny $nz $tnx $tny $tnz $hashsize $t0 $lambda $npert $ntemp $maxatt $irepo >> ${home}/mps-anneal/outputs/$advance
